...
@Injectable()
export class AstroportNeutronMainnetUsdcUsdtFarmInfoService implements FarmInfoService {
  ...
  refreshContractOnNetwork() {
    this.baseTokenContract = this.terrajs.settings.usdcToken;
    this.denomTokenContract = this.terrajs.settings.usdtToken;
    this.poolAprs = [
      {
        apr: 0,
    ...
    this.farmContract = this.terrajs.settings.astroportNeutronUsdcUsdtFarm;
    this.compoundProxyContract = this.terrajs.settings.astroportNeutronUsdcUsdtFarmCompoundProxy;
  ...
  async queryPairStats(poolInfos: Record<string, PoolInfo>, poolResponses: Record<string, PoolResponse>, govVaults: VaultsResponse, pairInfos: Record<string, PairInfo>, tokenInfos: Record<string, TokenInfo>, injPrice: number): Promise<Record<string, PairStat>> {
    ...
    const usdt = p.assets.find(a => a.info['native_token']?.['denom'] === this.denomTokenContract);
    const usdc = p.assets.find(a => a.info['native_token']?.['denom'] === this.baseTokenContract);
    if (!usdc || !usdt) {
      return;
    }
    pairs[key] = {
    ...
    pair.tvl = new BigNumber(usdc.amount).plus(usdt.amount)
      .times(depositAmount)
      .div(p.total_share)
      .toString();
