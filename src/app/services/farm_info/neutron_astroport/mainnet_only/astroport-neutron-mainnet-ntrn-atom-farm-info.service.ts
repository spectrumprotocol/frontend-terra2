...
@Injectable()
export class AstroportNeutronMainnetNtrnAtomFarmInfoService implements FarmInfoService {
  ...
  refreshContractOnNetwork() {
    ...
    this.denomTokenContract = ATOM_NEUTRON_MAINNET;
    this.poolAprs = [
      {
        apr: 0,
    ...
    this.farmContract = this.terrajs.settings.astroportNtrnAtomFarm;
    this.compoundProxyContract = this.terrajs.settings.astroportNtrnAtomFarmCompoundProxy;
    this.contractOnNetwork = this.terrajs.networkName;
  }

  // no LP APR calculation, return 0 to use Astroport API
  async queryPairStats(poolInfos: Record<string, PoolInfo>, poolResponses: Record<string, PoolResponse>, govVaults: VaultsResponse, pairInfos: Record<string, PairInfo>, tokenInfos: Record<string, TokenInfo>, ntrnPrice: number): Promise<Record<string, PairStat>> {
    ...
    const ntrn = p.assets.find(a => a.info['native_token']?.['denom'] === this.baseTokenContract);
    if (!ntrn) {
      return;
    }
    pairs[key] = {
