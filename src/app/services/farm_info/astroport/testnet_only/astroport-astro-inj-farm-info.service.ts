...
export class AstroportAstroInjFarmInfoService implements FarmInfoService {
  ...
  refreshContractOnNetwork() {
    ...
    this.denomTokenContract = Denom.INJ;
    this.poolAprs = [
      {
        apr: 0,
      ...
      {
        apr: 0,
        rewardSymbol: SYMBOLS.INJ,
        rewardContract: this.terrajs.settings.injToken
      }
    ];
    this.farmContract = this.terrajs.settings.astroportAstroInjFarm;
    this.compoundProxyContract = this.terrajs.settings.astroportAstroInjFarmCompoundProxy;
